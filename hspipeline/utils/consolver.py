...
def tally_values(csv_file, column_numbers=False, threshold=10):
    with open(csv_file, 'r') as f:
        ...
        for line in f:
            ...
            #values_dict = {
            #    key: list(set(values_dict[key] + [value])) for key,
            #    value in zip(column_names, values)
            #    if (key in values_dict) and (bool(threshold) == (len(values_dict[key]) < threshold))
            #}
            for key, value in zip(column_names, values):
                if (key in values_dict) and (bool(threshold) == (len(values_dict[key]) < threshold)):
                    values_dict[key] = list(set(values_dict[key] + [value]))
...
def separate_features(row, columns_numbers):
    return [
        [row[each] for each in columns_numbers],
        [row[each] for each in range(len(row)) if each not in columns_numbers]
    ]
...
def update_feature_dict(f_dict, features):
    """Increments the feature dictionary location by adding a new column."""
...
def consolidate(orig_file, id_names, feature_names=None):
    """Takes a CSV file and reformats it to consoldate rows
        by unique values of given id_names (given as a list).
    """
    ...
    repeated_feature_columns = {
        feat: [col]
        for feat, col in zip(feature_names, range(feat_start, (feat_start + len(feature_names))))
    }
    for repl in range(1,max_repl):
        head_append = [name + '_' + str(repl) for name in feature_names]
...
def check_feature_columns(feature_names):
    seen = set()
    seen_add = seen.add
    # Adds all elements not yet seen to seen and all other elements to seen_twice.
    seen_twice = set(x for x in feature_names if x in seen or seen_add(x))
    if len(seen_twice) > 0:
        print "Error: Feature name conflict for {}.".format(', '.join(seen_twice))
        return False
    return True
...
def make_choice(unique_id, feature_name, values, hist_values=False, initial=True, mode_default=False, underscore=False):
    ...
    # I'd like to update the hist_values... maybe?
    mode, mode_rep = mode_of_list(values)
    if mode_default:
        if mode_default not in [0,1]:
            print "Mode Default not recognized. Choose 0 for always, 1 for > 75%, or False for never."
            return ''
        if len(mode) == 1:
        ...
        if initial:
            print "{} has a conflict in {}\t{}.".format(str(unique_id), feature_name, ' : '.join([str(val) for val in values]))
        print "Choose: ".join([str(i) + " for " + str(val) for i, val in zip(range(len(set(values))), set(values))]) + " or N for Null or M for mode or U for user-defined."
        ...
        if choice in [str(i) for i in range(len(set(values)))]:
            ...
            if len(mode) > 1:
                print "There is more than one mode: {}.".format(', '.join(mode))
                return make_choice(unique_id, feature_name, values, hist_values, initial=False)
            else:
                return str(mode[0])
        elif choice in ['U']:
            if hist_values:
                print "Existing values for %s." % (feature_name)
                return make_choice(unique_id, feature_name, values=hist_values, initial=False)
            else:
...
def process_header(header, id_names, feature_names):
    """Takes the header and id/feature names and return the indices of the exact id_columns
...
def prompt_column_names(input_file):
    """Prompts the user to choose the unique id and features from a CSV header."""
    ...
    feature_names = []
    for name in header:
        print "Is %s a feature (F), part of the unique identifier (U), or neither (N)?" % (name)
        response = raw_input('-->')
        id_names, feature_names = check_feat_response(response, name, id_names, feature_names)
...
def check_duplicates(input_file, id_names, feature_names, log_file=None, id_indices= None, feature_indices=None, hist_values=False, mode_default=False, underscore=False):
    """Takes a CSV, like the one made by consolidate, which has unique_ids and possibly repeated features
        and gets user feedback to write a file in which the features of a unique id are consolidated."""
...
def console_and_resolve(csv_file, id_names=None, feature_names=None, log_file=None, get_feat=False):
    ...
    check_duplicates(cons_file, id_names, feature_names, log_file, id_col, feat_col, hist_values, mode_default=1, underscore=True)