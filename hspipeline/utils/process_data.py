...
#########put in a catch for categorical random variables
...
def numer_is_actually_categ(series):
	"""returns true if the guess is that the numberic seires is actually categorical"""
	return len(series.unique())<math.sqrt(len(series))

june='/mnt/data/education_data/dssg2015/dropout/vancouver/raw_data/june_1'
may='/mnt/data/education_data/dssg2015/dropout/vancouver/raw_data/may_28'


def find_each_file(direc):
    """moves recursively through a directory and returns the list of files"""
    file_list=[]
    for element in os.listdir(direc):
    	if os.path.isdir(element):
    		file_list=file_list+find_each_file(os.path.join(direc, element))
    	else:
    		file_list=file_list+[os.path.join(direc,element)]
    return file_list

def find_files(file_list,ftype=".csv"):
...
def get_files(direc, ftype,bad_extension):
	...
	file_list=find_each_file(direc)
	file_list=find_files(file_list, ftype)
...
def write_csv(df, name="desc.csv"):
	"""takes a dataframe  and writes a descriptive csv of it"""
	header=['variable','type','values_or_range','dist_or_std','mean_or_mode','median','number_missing']
	with open(os.path.splitext(name)[0]+'_variables.csv', 'w') as f:
		f.write(make_csv_line(header)+'\n')
		for variable in df.columns:
			f.write(make_csv_line(get_var_stats(df[variable]))+'\n')


def get_var_stats(series):
	"""returns categorical statistics or continuous statistics"""
	if check_numeric(series):
		return get_cont_stats(series)
	elif False:
		#to be converted if possible....
		return get_dt_stats(series)
	else:
		return get_categ_stats(series)
...
def get_categ_stats(series):
	return [series.name, series.dtype, summarize_list(series.unique()),summarize_list(categ_dist(series)),series.mode(),'median',pd.isnull(series).count()]

def get_cont_stats(series):
	return [series.name, series.dtype, [series.min(),series.max()],series.std(),series.mean(),'median',pd.isnull(series).count()]
...
def make_stats_dir(directory, ftype, bad_extension="_variables.csv"):
	file_names=get_files(directory, ftype, bad_extension)
	for f in file_names:
		write_csv(get_df(f), os.path.splitext(f)[0]+"_desc.csv")
